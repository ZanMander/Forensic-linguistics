...
def extract_docx_xml(docx_path):
    ...
    # Save the extracted XML file
    xml_path = "raw_document.xml"
...
def parse_document_xml(xml_data):
    ...
    rsid_timeline = []  # Track RSID appearance order
    ...
    for paragraph in root.findall('.//w:p', namespace):
        ...
        # Track RSID occurrences in order
        rsid_timeline.append(rsidR)
    ...
    return runs_data, rsid_colors, rsid_timeline
...
#Function to create a RSID timeline

def create_rsid_timeline(rsid_timeline):
    """Generates a timeline plot where x-axis is the order of appearance and y-axis is the RSID."""
    
    plt.figure(figsize=(12, 5))
    timestamps = list(range(len(rsid_timeline)))  # Use sequential order as time
    unique_rsids = list(set(rsid_timeline))  # Get unique RSIDs for y-ticks

    plt.scatter(timestamps, rsid_timeline, color='b', label="RSID Appearance", alpha=0.6)
    plt.plot(timestamps, rsid_timeline, linestyle="--", color="gray", alpha=0.5)  # Connect points

    # Label each RSID on the plot
    for i, rsid in enumerate(rsid_timeline):
        plt.text(timestamps[i], rsid, rsid, fontsize=9, ha="right", va="bottom")

    plt.xticks(timestamps, labels=timestamps, rotation=45)
    plt.yticks(unique_rsids, labels=unique_rsids)
    
    plt.xlabel("Time (Order of Appearance)")
    plt.ylabel("RSID")
    plt.title("RSID Timeline Illustration")
    plt.grid(axis="y", linestyle="--", alpha=0.5)

    timeline_path = "rsid_timeline.png"
...
def generate_report(docx_file):
    ...
    xml_data, xml_path = extract_docx_xml(docx_file)
    runs_data, rsid_colors, rsid_timeline = parse_document_xml(xml_data)

    rsid_stats = compute_rsid_stats(runs_data)

    # Generate both bar chart and heatmap and the timeline chart
    bar_chart_path = create_rsid_bar_chart(rsid_stats)
    heatmap_path = create_rsid_heatmap(rsid_stats)
    timeline_path = create_rsid_timeline(rsid_timeline)
    ...
    return rsid_stats, runs_data, rsid_colors, bar_chart_path, heatmap_path, timeline_path, report_path, xml_path
...
if uploaded_file:
    ...
    # Run analysis
    rsid_stats, runs_data, rsid_colors, bar_chart_path, heatmap_path, timeline_path, report_path, xml_path = generate_report(temp_path)
    ...
    # **Display RSID Timeline Chart**
    st.subheader("📍 RSID Timeline Illustration")
    ...
    with open(xml_path, "r", encoding="utf-8") as f:
        xml_data = f.read()
    st.download_button("📂 Download Raw XML File", xml_data, "raw_document.xml", "text/xml")
